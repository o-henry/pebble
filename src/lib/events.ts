...
export const DISCOVERY_STATUS_EVENT = "pebble://discovery-status";
...
export function listenToDiscoveryStatus(
  onStatus: (status: DiscoveryStatus) => void
): Promise<UnlistenFn> {
  if (!isTauriRuntime()) {
    return Promise.resolve(noop);
  }
  return listen<DiscoveryStatus>(DISCOVERY_STATUS_EVENT, (event) => {
    onStatus(event.payload);
  });
}
